import { React, ReactRedux, IMState, LayoutItemType, ContainerType } from 'jimu-core';
...
const useContainerMapId = (id: string): string => {
  return useSelector((state: IMState) => {
    ...
    const containerId = searchUtils.getParentWidgetIdOfWidgetOrSection(appConfig, id, LayoutItemType.Widget, browserSizeMode);
...
const useWhetherWidgetVisible = (id: string): boolean => {
  return useSelector((state: IMState) => {
    ...
    const info = searchUtils.getWidgetOrSectionContainerInfo(appConfig, id, LayoutItemType.Widget, browserSizeMode);
...
export const Runtime = (props: RuntimeProps) => {
  ...
  return <>
    ...
    {!mobile &&
      <>
        {onlyOpenOne && <SingleWidgetsLuncher
          onClose={closeWidget}
          ...
          placement={placement} />}
        {
          !onlyOpenOne && <MultipleWidgetsLuncher
            mode={displayType}
            start={start}
            spaceX={spaceX}
            spaceY={spaceY}
