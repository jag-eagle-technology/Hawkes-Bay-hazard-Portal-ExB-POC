...
import { Select, Radio, Label, ButtonGroup, Icon, Button, Switch, LinearUnit, AnchorDirection, NavigationVariant } from 'jimu-ui';
import { MenuNavigationStandard } from '../runtime/menu-navigation';
import { IconPicker } from 'jimu-ui/advanced/resource-selector';
import { InputUnit, NavStyleSettingByState, ComponentState } from 'jimu-ui/advanced/style-setting-components';
import { ThemeColorPicker } from 'jimu-ui/basic/color-picker';
import { useMenuNavigationVariant, getMenuNavigationVariant, useTranslate } from './utils';
...
const Setting = (props: SettingProps) => {
  ...
  const translateLocal = useTranslate(defaultMessage);
  const isRTL = useSelector((state: IMState) => state?.appContext?.isRTL);
  ...
  const { anchor, textAlign, icon, submenuMode, gap } = standard || {} as MenuNavigationStandard;
  ...
  return <div css={style} className="widget-setting-menu jimu-widget-setting">
    ...
    <SettingSection title={translate('appearance')}>
      ...
      <SettingRow label={translate('space')} flow="no-wrap">
        <InputUnit value={gap} onChange={(value) => onSettingConfigChange(['standard', 'gap'], `${value.distance}${value.unit}`)} />
      </SettingRow>

      <SettingRow flow="no-wrap" label={translate('alignment')}>
        <ButtonGroup>
          <Button title={translate(!isRTL ? 'left' : 'right')} type="secondary" icon size="sm" active={textAlign === 'left'}
            onClick={() => onSettingConfigChange(['standard', 'textAlign'], 'left')}>
            <Icon icon={leftIcon}></Icon>
          </Button>
          <Button title={translate('center')} type="secondary" icon size="sm" active={textAlign === 'center'}
            onClick={() => onSettingConfigChange(['standard', 'textAlign'], 'center')}>
            <Icon icon={centerIcon}></Icon>
          </Button>
          <Button title={translate(!isRTL ? 'right' : 'left')} type="secondary" icon size="sm" active={textAlign === 'right'}
            onClick={() => onSettingConfigChange(['standard', 'textAlign'], 'right')}>
