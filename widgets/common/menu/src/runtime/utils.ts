...
import { LinkTarget, NavigationItem, AnchorDirection, componentStyleUtils, NavigationVariant, styleUtils } from 'jimu-ui';
...
export const useAvtivePage = () => {
  const currentPage = useSelector((state: IMState) => state.appRuntimeInfo?.currentPageId);

  return useCallback((item: NavigationItem) => {
    return getPageId(item) === currentPage;
  }, [currentPage])
}
 ...
 * @param anchor
 */
export const useAnchor = (anchor: AnchorDirection) => {
  const mobile = useSelector((state: IMState) => state?.browserSizeMode === BrowserSizeMode.Small)
  return !mobile ? anchor : 'full';
}
...
export const useNavAdvanceStyle = (advanced: boolean, menuStyle: ThemeNavType, variant: NavigationVariant, vertical?: boolean) => {
  return useMemo(() => {
    ...
    &.jimu-nav {
      ...
      ${styleUtils.getIconBtnStyleByState(variant?.item)}
    }
`;
